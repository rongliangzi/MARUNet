...
class UpNet(nn.Module):
    def __init__(self, load_model='', downsample=1, bn=False, NL='relu', objective='dmp', sp=False, se=False, block=''):
        ...
        self.reduc3 = conv_act(512, 256, 1)
        self.back3 = make_layers([256,], in_channels=1280, dilation=True, batch_norm=bn, NL=self.NL, se=se)
        self.reduc2 = conv_act(512, 256, 1)
        self.back2 = make_layers([128], in_channels=768, dilation=True, batch_norm=bn, NL=self.NL, se=se)
        self.back1 = make_layers([128], in_channels=512, dilation=True, batch_norm=bn, NL=self.NL, se=se)
        self.back0 = make_layers([64], in_channels=320, dilation=True, batch_norm=bn, NL=self.NL, se=se)
    ...
    def forward(self, x_in):
        ...
        x4u = F.interpolate(x4, size=x3.shape[2:])#1/8size, 512
        x4u = self.reduc3(x4u)#1/8size, 256
        xb3 = F.interpolate(xb4, size=x3.shape[2:]) #1/8 size, 512
        xb3 = torch.cat([x3, xb3, x4u], dim=1) #1/8 size, 1280
        xb3 = self.back3(xb3) #1/8 size, 256
        
        xb2 = F.interpolate(xb3, size=[x_in.shape[2]//4, x_in.shape[3]//4]) #1/4 size, 256
        x3u = F.interpolate(x3, x2.shape[2:])#1/4size,512
        x3u = self.reduc2(x3u)#1/4size,256
        xb2 = torch.cat([x2, xb2, x3u], dim=1) #1/4 size, 768
        xb2 = self.back2(xb2) #1/4 size, 128
        
        xb1 = F.interpolate(xb2, size=x1.shape[2:]) #1/2 size, 128
        x2u = F.interpolate(x2, x1.shape[2:])#1/2size,256
        xb1 = torch.cat([x1, xb1, x2u], dim=1)#1/2, 512
        xb1 = self.back1(xb1)#1/2, 128
        
        xb0 = F.interpolate(xb1, size=x_in.shape[2:]) #1 size, 64
        x1u = F.interpolate(x1, size=x_in.shape[2:])#1 size, 128
        xb0 = torch.cat([x0, xb0, x1u], 1)#1,320
