...
class M_VGG7(nn.Module):
    def __init__(self, load_model='', downsample=1, bn=False, NL='relu', objective='dmp', sp=False, se=False, block=''):
        ...
        if not self.block:
            self.backconv1 = make_layers([256, 128], in_channels=256, dilation=True, batch_norm=bn, NL=self.NL, se=se, groups=0)
            self.backconv2 = make_layers([128], in_channels=128, dilation=True, batch_norm=bn, NL=self.NL, se=se, groups=0)
            self.backconv3 = make_layers([64], in_channels=128, dilation=True, batch_norm=bn, NL=self.NL, se=se, groups=0)
