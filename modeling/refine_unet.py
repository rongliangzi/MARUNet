...
class RefUNet(nn.Module):
    ...
    def forward(self, x_in):
        ...
        if self.objective == 'dmp+amp':
            ...
            d4 = amp * d4
            db = amp * db
            return torch.abs(dmp), torch.abs(d0), torch.abs(d1), torch.abs(d2), torch.abs(d3), torch.abs(d4), torch.abs(db), amp
        else:
            x = self.output_layer(xb0)
            x_ref = self.refine(x)
            return torch.abs(x_ref), torch.abs(x),torch.abs(d1), torch.abs(d2), torch.abs(d3), torch.abs(d4), torch.abs(db)
