...
class Post_Prob(Module):
    ...
    def forward(self, points, st_sizes):
        ...
        # num_points_per_image:[k(number of points in this cropped patch)]
        # all_points shape:[k,2], cood
        if len(all_points) > 0:
            ...
            for dis, st_size in zip(dis_list, st_sizes):
                if len(dis) > 0:
                    ...
                    if self.sigma == 0:
                        cur_sigma = all_points[:, 2] * 0.3
                        cur_sigma.clamp_(3, 8)
                        #print(cur_sigma.mean())
                        cur_sigma = torch.cat([cur_sigma, torch.FloatTensor([8.0]).to(self.device)], dim=0)
                        cur_sigma.unsqueeze_(1)
                    else:
                        cur_sigma = self.sigma
                    #dis = -dis / (2.0 * self.sigma ** 2)
                    #prob = self.softmax(dis)
                    
                    dis = - dis / (2.0 * cur_sigma ** 2)
                    dis.exp_()
                    dis = dis/(dis.sum(dim=1).unsqueeze_(1))
                    dis[-1,dis.sum(dim=0)==0.0]+=1e-10
                    prob = (dis)/(dis.sum(dim=0).unsqueeze_(0))
                else:
                    prob = None
